...
// Mock the promotionController functions with `status: "success"`
jest.mock("../src/api/v1/controllers/promotionController", () => ({
    getAllPromotions: jest.fn((req, res) => res.status(200).send({ message: "Promotions Retrieved", data: [], status: "success" })),
    getPromotionById: jest.fn((req, res) => res.status(200).send({ message: "Promotion with id '1' retrieved successfully", data: {}, status: "success" })),
    createPromotion: jest.fn((req, res) => res.status(201).send({ message: "Promotion Created", data: {}, status: "success" })),
    updatePromotion: jest.fn((req, res) => res.status(200).send({ message: "Promotion Updated", data: {}, status: "success" })),
    deletePromotion: jest.fn((req, res) => res.status(200).send({ message: "Promotion Deleted", status: "success" })),
}));
...
describe("Promotion Routes", () => {
    ...
    describe("PUT /api/v1/promotions/:id", () => {
        ...
        it("should return a 200 status with a success message", async () => {
            ...
            const mockId = "1"; 
            const response = await request(app).put(`/api/v1/promotions/${mockId}`).send(mockPromotion);
            expect(response.status).toBe(200);
